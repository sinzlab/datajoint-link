...
class TableFactory(Base):
    ...
    def _create_table_cls(self) -> Type[Lookup]:
        ...
        return self.config.schema(table_cls)

    def _create_master_table_cls(self) -> Type[Lookup]:
        # noinspection PyTypeChecker
        return type(self.config.table_name, (Lookup,), dict(definition=self.config.table_definition))
    ...
    def _create_non_flag_part_table_classes(self, table_cls_attrs: Dict[str, Type[Part]]) -> None:
        table_cls_attrs.update(self._create_part_table_classes(self.config.part_table_definitions))
