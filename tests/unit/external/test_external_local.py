...
class TestCreateTableCls:
    ...
    def test_if_source_part_tables_are_assigned_to_created_table_cls(self, factory, source_table_part_names):
    ...
    def test_if_part_tables_are_part_tables(self, factory, source_table_part_names):
        ...
        for part_name in source_table_part_names:
            part = getattr(local_table_cls, part_name)
            assert issubclass(part, Part)

    def test_if_definition_of_part_tables_is_correct(self, factory, source_table_part_names):
        local_table_cls = factory.create_table_cls()
        for part_name in source_table_part_names:
            part = getattr(local_table_cls, part_name)
            assert part.definition == f"-> master\n{part_name}_heading_with_replaced_stores"
